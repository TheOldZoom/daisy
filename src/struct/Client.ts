...
class Client extends DiscordClient {
    ...
    async loadCommands(commandsPath?: string) {
        ...
        try {
            ...
            this.logs.error('Error loading commands', error instanceof Error ? error.message : String(error));
        }
    }


    async loadEvents(eventsPath?: string) {
        const resolvedPath = eventsPath ??
            path.join(__dirname, '../events');


        try {
            const eventFolders = fs.readdirSync(resolvedPath)
                .filter(folder =>
            ...
            for (const eventFolder of eventFolders) {
                ...
                for (const file of eventFiles) {
                    ...
                    const handler = eventModule.default?.execute ||
                        eventModule.execute ||
                        eventModule.default;
                    ...
                    this.on(eventFolder, (...args) => {
                        eventHandlers.forEach(handler => {
                            try {
                                ...
                                this.logs.error(`Error in ${eventFolder} event handler`,
            ...
            this.logs.error('Error loading events', error instanceof Error ? error.message : String(error));
        }
    }
