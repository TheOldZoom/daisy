    ...
    subs: [
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                ...
                if (args.length < 2) {
                    embed.setDescription('Please provide both a status type and text.\nUsage: `status add <type> <text> [url]`\nValid types: Playing, Watching, Listening, Competing, Streaming');
                    await message.reply({ embeds: [embed] });
                    return;
                }

                const type = args[0];
                // In your validTypes array
                const validTypes = ['Playing', 'Watching', 'Listening', 'Competing', 'Streaming', 'Custom'];


                if (!validTypes.includes(type)) {
                    embed.setDescription('Invalid status type. Valid types are: Playing, Watching, Listening, Competing, Streaming');
                    await message.reply({ embeds: [embed] });
                    return;
                }
        ...
        },
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                ...
                if (isNaN(id)) {
                    embed.setDescription('Please provide a valid numeric ID.')
                ...
                try {
                    ...
                    embed.setDescription(`Removed status: "${status.text}" (${status.type})`);
                } catch (error) {
                    embed.setDescription(`Status with ID ${id} not found.`)
                        .setColor(Colors.hotPinkPop);
                }
        ...
        },
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                ...
                if (statuses.length === 0) {
                    embed.setDescription('There are currently no statuses set for the bot.')
                        .setColor(Colors.hotPinkPop);
                } else {
        ...
        },
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                ...
                if (statuses.length === 0) {
                    embed.setDescription('There are no statuses to update to.')
                        .setColor(Colors.hotPinkPop);
                    await message.reply({ embeds: [embed] });
                    return;
                }
                ...
                embed.setDescription(`Status updated to: "${processedStatus}" (${randomStatus.type})`);
                await message.reply({ embeds: [embed] });
            }
        }
    ]
