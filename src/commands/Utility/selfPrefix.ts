...
export default new Command({
    ...
    execute: async (message, args, client): Promise<void> => {
        ...
        if (data?.selfprefix) {
            ...
            embed.setDescription("You do not have a self prefix set.").setColor(Colors.hotPinkPop)
        }
    ...
    subs: [
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                ...
                if (prefix.length > 3) {
                    embed.setDescription("Your prefix must not be longer than 3 characters.");
                    await message.reply({ embeds: [embed] });
                    return;
                }
