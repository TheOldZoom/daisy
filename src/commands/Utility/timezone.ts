...
export default new Command({
    ...
    execute: async (message, args, client): Promise<void> => {
        ...
        const userTime = moment().tz(data.timezone).format("dddd, MMMM Do, hh:mm A")
        embed.setDescription(`**${member.user.tag}'s** current time **${userTime}**`);
        await message.reply({ embeds: [embed] });
    },
    subs: [
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                if (!args[0]) {
                    const embed = new EmbedBuilder().setDescription("Please provide a valid timezone.");
                    await message.reply({ embeds: [embed] });
                    return;
                }
                ...
                if (!moment.tz.zone(timezone)) {
                    const embed = new EmbedBuilder().setDescription("That is not a valid timezone. Please provide a valid timezone.");
                    await message.reply({ embeds: [embed] });
                    return;
                }
                ...
                const embed = new EmbedBuilder().setDescription(`Your timezone has been set to **${timezone}**.`);
                await message.reply({ embeds: [embed] });
            }
        },
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                ...
                const embed = new EmbedBuilder().setDescription("Your timezone has been removed.");
                await message.reply({ embeds: [embed] });
            }
        }
    ]
