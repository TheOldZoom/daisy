...
export default new Command({
    ...
    execute: async (message, args, client): Promise<void> => {
        ...
        if (data?.prefix) {
            embed.setDescription(`The prefix for this server is **\`${data.prefix}\`**.`);
        } else {
            embed
    ...
    subs: [
        {
            ...
            execute: async (message, args, client): Promise<void> => {
                ...
                if (prefix.length > 3) {
                    embed.setDescription('The prefix must not be longer than 3 characters.');
                    await message.reply({ embeds: [embed] });
                    return;
                }
                ...
                embed.setDescription(`The prefix for this server has been set to \`${prefix}\`.`);
                await message.reply({ embeds: [embed] });
            },
        },
        {
